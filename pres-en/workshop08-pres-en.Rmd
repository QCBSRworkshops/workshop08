...
???
- A linear model can easily accomodate certain types of non-linear responses (e.g. $x^2$) but this approach strongly relies on (arbitrary or well-informed) choices, and is less flexible than using and additive model.
...
We will now take a few minutes to look at what GAMs are doing behind the scenes. Lets first consider a simple model containing one smooth function $f$ of one covariate, $x$:
...
`k` ![:faic](arrow-right) basis dimension
  ...
  - the default $k$ for is 10 for `s()`, and 5 for `te()` and `ti()`.

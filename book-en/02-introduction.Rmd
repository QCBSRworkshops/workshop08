...
As we mentioned briefly in [Chapter 2](#intro-linear-models), we could manually specify a linear model with multiple predictor variables to try to accommodate certain types of non-linear responses. For example, we could try to build this linear model with multiple predictors:

$$y_i = \beta_0 + \beta_1(x_{1,i}) + \beta_2(x_{2,i}) + ... + \epsilon$$
However, the fit of this model would be determined manually based on the modeller's decisions, and would quickly become difficult to work with. One big advantage of using an additive model, such as a GAM, over a manual specification of an multiple-predictor regression model is that the fitting method (usually maximum likelihood) automatically determines the optimal shape of the curve fit. This optimal shape is, in other words,  *the degree of smoothness* of $f(x)$.

:::explanation

Briefly, GAMs are effectively a nonparametric form of regression where the $\beta x_i$ of a linear regression is replaced by an arbitrary smooth function of the explanatory variables, $f(x_i)$, and the model becomes:
...
Here, the AIC of the nested GAM is lower, which indicates that adding a smoothing function improves model performance. Linearity is therefore not supported by our data.
...
:::explanation
We have not discussed effective degrees of freedom (**EDF**) yet, but these are a key tool to help us interpret the fit of a GAM. More on this in the next sections.
:::

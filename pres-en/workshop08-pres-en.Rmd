...
Let's look at an example. First, we will download the `ISIT` datasets.
...
data_plot<-ggplot(isit2, aes(y=Sources, x=SampleDepth)) +geom_point()+theme_bw()
data_plot
```
---
# GAM
...
```{r, eval=TRUE,echo = FALSE}
gam_model1 <- gam(Sources ~ s(SampleDepth),data=isit2)
data_plot <- data_plot +
...
.comment[Note: as opposed to one fixed coefficient, $β$ in linear regression, the smooth function can continually change over the range of the predictor x]
...
.comment[Note that the model `y~s(x)` gives exactly the same results as `y~s(x)+x`. We used the `s(x)+x` to illustrate the nestedness of the model, but the `+x` can be omitted.]
...
.comment[`Season` need to be converted into a factor] 

---
# GAM with multiple variables

Let's start with a basic model, with one smoothed term (`SampleDepth`) and one categorical predictor (`Season`, which has 2 levels).
```{r,eval=TRUE,echo=FALSE}
isit$Season <- as.factor(isit$Season)
```
...
> In our basic model the edf of smooth function s(SampleDepth) is ~8, which suggests a highly non-linear curve.
...
As before, we can perform an AIC to test if the smoothed term is necessary
